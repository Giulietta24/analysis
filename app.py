...
@st.cache_data(ttl=300)
def fetch_ticker_metrics(ticker_str: str, is_index: bool = False) -> dict:
    ...
    try:
        ...
        return {"Name": name, "Price": price_fmt, "Cap_Raw": mcap, "Market Cap": mcap_fmt, "P/E": pe_fmt, "Growth": growth_fmt}
    except Exception as e:
        st.sidebar.warning(f"⚠️ Tracking warning for '{ticker_str}': Missing dynamic data points.")
        return fallback_dict
...
if st.sidebar.button("Run Financial Evaluation Matrix"):
    ...
    with st.spinner("Fetching underlying real-time exchange data..."):
        ...
        t3 = fetch_ticker_metrics(custom_index, is_index=True)
        ...
        b_cap = fetch_ticker_metrics(cap_ticker, is_index=True)
        b_eq = fetch_ticker_metrics(eq_ticker, is_index=True)

        matrix_data = {
            "Metric": ["Asset Name", "Share Price", "Market Size / AUM", "P/E Ratio", "TTM Performance / Growth"],
            ...
            f"Cap-Weight ({cap_ticker})": [b_cap["Name"], "N/A", b_cap["Market Cap"], b_cap["P/E"], b_cap["Growth"]],
            f"Equal-Weight ({eq_ticker})": [b_eq["Name"], "N/A", b_eq["Market Cap"], b_eq["P/E"], b_eq["Growth"]]
        }
        
        df = pd.DataFrame(matrix_data).set_index("Metric")
        st.dataframe(df, use_container_width=True)
        
        st.download_button(label="📥 Export Matrix Dataset to CSV", data=df.to_csv().encode('utf-8'), file_name="dashboard_export.csv", mime="text/csv")
